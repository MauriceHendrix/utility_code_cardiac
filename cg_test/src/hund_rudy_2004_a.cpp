...
#include "MathsCustomFunctions.hpp"
    ...
    double Cellhund_rudy_2004_aFromCellML::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_INaCa__denom2 = pow(var_INaCa__KmNao, 3.0) * var_INaCa__ca_i_NaCa + pow(var_chaste_interface__Na__Na_i, 3.0) * var_INaCa__KmCao + pow(var_INaCa__KmNai, 3.0) * (1.0 + var_INaCa__ca_i_NaCa / var_INaCa__KmCai) * var_Environment__Ca_o; // mM4
        const double var_INaCa__denom3 = pow(var_Environment__Na_o, 3.0) * var_INaCa__ca_i_NaCa + pow(var_chaste_interface__Na__Na_i, 3.0) * var_Environment__Ca_o + pow(var_Environment__Na_o, 3.0) * (1.0 + pow((var_chaste_interface__Na__Na_i / var_INaCa__KmNai), 3.0)) * var_INaCa__KmCai; // mM4
        ...
        const double var_INaK__INaK = 1.0 * var_Environment__K_o * var_INaK__fnak * var_INaK__ibarnak / ((1.0 + pow((var_INaK__kmnai / var_chaste_interface__Na__Na_i), 2.0)) * (var_Environment__K_o + var_INaK__kmko)); // uA_per_uF
    ...
    void Cellhund_rudy_2004_aFromCellML::EvaluateYDerivatives(double var_chaste_interface__Environment__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
        ...
        const double var_Ca__bss = 1.0 / (1.0 + pow((var_chaste_interface__Ca__Ca_ss + var_Ca__KmBSL), (-2.0)) * var_Ca__BSLmax * var_Ca__KmBSL + pow((var_chaste_interface__Ca__Ca_ss + var_Ca__KmBSR), (-2.0)) * var_Ca__BSRmax * var_Ca__KmBSR); // dimensionless
        ...
        const double var_Ca__bmyo = 1.0 / (1.0 + pow((var_chaste_interface__Ca__Ca_i + var_Ca__kmc), (-2.0)) * var_Ca__cbar * var_Ca__kmc + pow((var_chaste_interface__Ca__Ca_i + var_Ca__kmt), (-2.0)) * var_Ca__kmt * var_Ca__tbar); // dimensionless
        const double var_Environment__Ca_o = 1.8; // mM
        const double var_Environment__Cl_o = 100.0; // mM
        const double var_Environment__F = 96485.0; // C_per_mole
        const double var_Environment__FonRT = 0.0374358835078; // per_mV
        ...
        const double var_ICaL__tauf = 1.0 / (0.052900000000000003 + 0.24110000000000001 * exp(-0.19019454876899997 * pow((-1 + 0.10318426646304972 * var_chaste_interface__cell__V), 2.0))); // ms
        const double d_dt_chaste_interface_var_ICaL__f = (-var_chaste_interface__ICaL__f + var_ICaL__fss) / var_ICaL__tauf; // 1 / ms
        const double var_ICaL__tauf2 = 1.0 / (0.0054000000000000003 + 0.042299999999999997 * exp(-1.20074125971556 * pow((-1 + 0.053842757610673787 * var_chaste_interface__cell__V), 2.0))); // ms
        ...
        const double var_Irel__ross = var_Irel__dro_inf / (1.0 + 1.0 * pow(1 / var_ICaL__ICaL, 2.0)); // dimensionless
        ...
        const double var_INa__INa = (-var_reversal_potentials__ENa + var_chaste_interface__cell__V) * var_INa__G_Na_mult * var_INa__gNa; // uA_per_uF
        const double var_INal__INal = pow(var_chaste_interface__INal__mL, 3.0) * (-var_reversal_potentials__ENa + var_chaste_interface__cell__V) * var_INal__GNaL * var_chaste_interface__INal__hL; // uA_per_uF
        const double var_Na__CTNaCl = pow((-var_reversal_potentials__ECl + var_reversal_potentials__ENa), 4.0) * var_Na__CTNaClmax / (59494197.640692554 + pow((-var_reversal_potentials__ECl + var_reversal_potentials__ENa), 4.0)); // mM_per_ms
