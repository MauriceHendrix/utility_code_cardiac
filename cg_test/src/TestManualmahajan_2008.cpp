...
#include "MathsCustomFunctions.hpp"
    ...
    boost::shared_ptr<RegularStimulus> TestManualmahajan_2008FromCellML::UseCellMLDefaultStimulus()
    {
        // Use the default stimulus specified by CellML metadata
        const double var_chaste_interface__cell__stim_amplitude = -14.999999999999998 * HeartConfig::Instance()->GetCapacitance(); // uA_per_uF
    ...
    double TestManualmahajan_2008FromCellML::GetIntracellularCalciumConcentration()
    {
        return mStateVariables[1];
    }
    
    TestManualmahajan_2008FromCellML::TestManualmahajan_2008FromCellML(boost::shared_ptr<AbstractIvpOdeSolver> pSolver, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
        : AbstractCardiacCell(
                pSolver,
                26,
                0,
                pIntracellularStimulus)
    {
        ...
        // 
        this->mpSystemInfo = OdeSystemInformation<TestManualmahajan_2008FromCellML>::Instance();
        Init();
    ...
    TestManualmahajan_2008FromCellML::~TestManualmahajan_2008FromCellML()
    {
    }
    
    double TestManualmahajan_2008FromCellML::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_chaste_interface__i_ionic = (0.9999999999999999*var_cell__Itotal) * HeartConfig::Instance()->GetCapacitance(); // uA_per_cm2
    ...
    void TestManualmahajan_2008FromCellML::EvaluateYDerivatives(double var_chaste_interface__Environment__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
        // Inputs:
        // Time units: millisecond
        double var_chaste_interface__cell__V = (mSetVoltageDerivativeToZero ? this->mFixedVoltage : rY[0]);
        ...
        else
        {
            ...
            const double var_IKr__rg = 1.0 / (1.0 + 4.363237316886145 * exp(0.044642857142857144 * var_chaste_interface__cell__V)); // dimensionless
            const double var_Ito__rs_inf = 1.0 / (1.0 + exp(var_Ito__rt2)); // dimensionless
            const double var_cell__i_Stim = 1.0000000000000002 * GetIntracellularAreaStimulus(var_chaste_interface__Environment__time)/ HeartConfig::Instance()->GetCapacitance(); // nA_per_nF
            const double var_ICaL__xica = 2.0 * var_ICaL__jca * var_cell__wca; // nA_per_nF
            const double var_reversal_potentials__K_i = 140.0; // mM
            const double var_reversal_potentials__ek = 1.0 * log(var_Environment__K_o / var_reversal_potentials__K_i) / var_Environment__FonRT; // mV
            const double var_IK1__aki = 1.02 / (1.0 + 7.35454251046446e-07 * exp(0.2385 * var_chaste_interface__cell__V - 0.2385 * var_reversal_potentials__ek)); // per_ms
...
template<>
void OdeSystemInformation<TestManualmahajan_2008FromCellML>::Initialise(void)
{
